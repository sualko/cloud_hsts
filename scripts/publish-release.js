...
const files = [
	path.join(__dirname, '..', 'archives', `hsts-v${package.version}.tar.gz`),
	path.join(__dirname, '..', 'archives', `hsts-v${package.version}.tar.gz.asc`),
	path.join(__dirname, '..', 'archives', `hsts-v${package.version}.tar.gz.ncsig`),
	path.join(__dirname, '..', 'archives', `hsts-v${package.version}.tar.gz.sig`),
];
...
async function generateChangelog() {
	...
	function stringifyEntry(entry) {
		const issues = entry.issues.map(issue => {
			return `[#${issue}](https://github.com/sualko/cloud_hsts/issues/${issue})`;
		}).join('');
...
async function createGithubRelease(changeLog) {
	...
	const octokit = new Octokit({
		...
		userAgent: 'custom releaser for sualko/cloud_hsts',
	});
